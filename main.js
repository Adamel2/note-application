function addNote(event) {
    ...
    const newNote = document.getElementById("content-add").value;
    ...
    if (
        ...
        document.getElementById(
            "list"
        ).innerHTML += `<li id=${key} onmouseover="unHidden(this.id)" onmouseout="appendHiddenAttribute(this.id)"><div id="style-6" class="item-div">${JSON.parse(localStorage.getItem(key)).title
...
window.onload = () => {
    ...
    listItems.forEach((element) => {
        document.getElementById("list").innerHTML += `<li id=${element.id
            } onmouseover="unHidden(this.id)" onmouseout="appendHiddenAttribute(this.id)"><div id="style-6" class="item-div">${element.title
            } </div><input class="input-date" type="text" disabled value=${new Date(element.date).toLocaleDateString() +
            "-" +
...
function unHidden(id) {
    const myItem = document.getElementById(id);
    const buttonChild = myItem.querySelector("button");
